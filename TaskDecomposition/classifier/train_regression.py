...
def main(args):
    ...
    X = df[feature_cols]
    y = df["label"]  # Use raw float labels for regression (no clipping/rounding)
    sd = df["sd"]
    ...
    # Train on raw float labels
    model.fit(X, y)

    # Predict raw floats (no clipping or rounding)
    ...
    print("\n=== TRAIN METRICS (XGBRegressor + GPT features) ===")
    print(f"MSE:                 {mse:.4f}")
    print(f"Spearman ρ:          {spearman:.4f}")
    print(f"Acc within SD or 1:  {acc_sd1:.4f}")

    joblib.dump(model, "xgb_regression.pkl")
    print("Saved model → xgb_regression.pkl")
